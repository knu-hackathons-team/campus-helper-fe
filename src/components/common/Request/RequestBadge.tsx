...
export const StatusBadge = ({ status, className = '' }: StatusBadgeProps) => {
  ...
  return (
    <span
      className={`px-2 py-1 rounded-full text-xs ${config.color} ${className} flex items-center gap-1`}
    >
      <Icon size={12} />
      {config.text}
    </span>
...
export const UserBadge = ({ type, className = '' }: UserBadgeProps) => {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs ${config[type].color} ${className} flex items-center gap-1`}
    >
      <Icon size={12} />
      {config[type].text}
    </span>
...
export const GroupFundingBadge = ({
  ...
  <span
    className={`px-2 py-0.5 bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200 rounded-full text-xs flex items-center gap-1 ${className}`}
  >
    <Users size={12} />
    공동요청 가능
  </span>
